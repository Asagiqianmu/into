...
var SmokeRecord = function (_wepy$page) {
    _inherits(SmokeRecord, _wepy$page);
    ...
    function SmokeRecord() {
        ...
        _classCallCheck(this, SmokeRecord);

        for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
            args[_key] = arguments[_key];
        }

        return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = SmokeRecord.__proto__ || Object.getPrototypeOf(SmokeRecord)).call.apply(_ref, [this].concat(args))), _this), _this.data = {
            list: [],
                ...
                success: function success(res) {
                    ...
                    wx.request({
                        ...
                        success: function success(res) {
                            ...
                            if (data.code != 200) {
                                wx.redirectTo({
                                    url: 'login?page=smoke-record'
                                });
                            } else {
                                wx.request({
                                    url: 'https://api.yingtong-pm.cn/v1/client/smokerecords',
                                    method: 'GET',
                                    data: {
                                        token: token,
                                    ...
                                    success: function success(res) {
                                        ...
                                        if (data.code == 200) {
                                            for (var i = 0; i < data.data.length; i++) {
                                                var obj = {};
                                                obj.time = _util.formatDate.call(new Date(data.data[i].createTime.time), 'hh:mm:ss');
                                                obj.houseid = data.data[i].houseId;
                                                obj.num = data.data[i].num;
                                                obj.status = data.data[i].status;
                                                self.list.push(obj);
                                                self.$apply();
                                            }
                                        } else {
...
Page(require('./../npm/wepy/lib/wepy.js').default.$createPage(SmokeRecord , 'pages/smoke-record'));
